...
test("registered routes persist through migrated SQLite repositories and vault import", async (t) => {
  ...
  assert.equal((await request("settings:get")).bilateralStimulation.fieldMode, "light");
  assert.equal((await request("settings:update-bilateral-stimulation", { speed: 0.5 })).speed, 0.5);
  assert.equal((await request("settings:update-bilateral-stimulation", { fieldMode: "dark" })).fieldMode, "dark");
  await assert.rejects(
    () => request("settings:update-bilateral-stimulation", { speed: 0.4 }),
    /Expected speed to be between 0.5 and 1.2/
  );
  await assert.rejects(
    () => request("settings:update-bilateral-stimulation", { fieldMode: "system" }),
    /Expected fieldMode to be a field mode/
  );
